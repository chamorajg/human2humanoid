    ...
    def __init__(
        ...
        if np.any(self.lowcut):
            if not np.all(self.lowcut):
                raise ValueError(
                    "All the filters must be of the same type: "
                    "lowpass or bandpass"
                )
            self.ftype = "bandpass"
        else:
...
class ActionFilterButterTorch(ActionFilterButter):
    ...
    def filter(self, x):
        ...
        self.xhist_torch = torch.cat(
            [x.unsqueeze(0), self.xhist_torch[:-1]], dim=0
        )
        self.yhist_torch = torch.cat(
            [y.unsqueeze(0), self.yhist_torch[:-1]], dim=0
        )
        return y

    def reset_hist(self, action_ids):
        self.xhist_torch[:, action_ids] = 0.0
        self.yhist_torch[:, action_ids] = 0.0
...
class ActionFilterExp(ActionFilter):
    ...
    def __init__(self, alpha, num_joints):
        ...
        for a in self.alphas:
            a_coeffs.append(np.asarray([1.0, a - 1.0]))
