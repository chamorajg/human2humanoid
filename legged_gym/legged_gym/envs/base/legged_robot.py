...
class LeggedRobot(BaseTask):
    def __init__(
        ...
        if self.cfg.control.action_filt:
            self.action_filter = ActionFilterButterTorch(
                ...
                highcut=np.ones(self.num_envs * self.num_actions)
                * self.cfg.control.action_cutfreq,
        ...
        if self.cfg.motion.teleop:
            ...
            self._track_bodies_id = [
                ...
                for body_name in self.cfg.motion.teleop_selected_keypoints_names
            ]
            self._track_bodies_extend_id = self._track_bodies_id + [
                len(self._body_list),
                len(self._body_list) + 1,
            ]
            self.extend_body_pos = (
                torch.tensor([[0.3, 0, 0], [0.3, 0, 0]])
            ...
            if self.cfg.motion.extend_head:
                ...
                self.extend_body_pos = (
                    torch.tensor([[0.3, 0, 0], [0.3, 0, 0], [0, 0, 0.75]])
                    .repeat(self.num_envs, 1, 1)
                    .to(self.device)
                )
    ...
    def setup_kin_info(self):
        if self.cfg.train.distill:
            ...
            if self.cfg.train.algorithm.get("save_z_noise", False):
                self.kin_dict["z_noise"] = torch.zeros(
                    [self.num_envs, self.cfg.train.policy.embedding_size]
                ).to(self.device)
    ...
    def load_expert(self):
        ...
        cfg.motion.teleop_obs_version = (
            self.cfg.train.distill_model_config.obs_v
        )
        cfg.motion.future_tracks = (
            self.cfg.train.distill_model_config.future_tracks
        )
        cfg.motion.num_traj_samples = (
        ...
        load_path = os.path.join(
            ...
            "logs",
            cfg.task,
            cfg.train.dagger.load_run_dagger,
            f"model_{cfg.train.dagger.checkpoint_dagger}.pt",
        )
        ...
        loading_keys = [
            k
            for k in model_dict[model_key].keys()
            if k.startswith(net_key_name)
        ]
        self.expert_policy = load_mlp(
            loading_keys, model_dict, actvation_func, model_key=model_key
        )
    ...
    def step(self, actions):
        ...
        if self.cfg.train.distill and not self.cfg.env.test:
            ...
            if "expert_policy" in self.__dict__:
                (
                    temp_obs_v,
                    temp_future_tracks,
                    temp_num_traj_samples,
                    teleop_selected_keypoints_names,
                    temp_num_privileged_obs,
                    temp_num_observations,
                    temp_add_noise,
                    temp_track_bodies_extend_id,
                ) = (
                    self.cfg.motion.teleop_obs_version,
                    self.cfg.motion.future_tracks,
                ...
                self.cfg.env.num_observations = (
                    self.cfg.train.distill_model_config.num_observations
                )

                _track_bodies_extend_id_new = [
                    self._body_list.index(body_name)
                    for body_name in self.cfg.motion.teleop_selected_keypoints_names
                ] + [len(self._body_list), len(self._body_list) + 1]
        ...
        if self.cfg.control.action_filt:
            actions = self.action_filter.filter(
                actions.reshape(self.num_envs * self.num_actions)
        ...
        for _ in range(self.cfg.control.decimation):
            self.torques = self._compute_torques(actions).view(
                self.torques.shape
            )
        ...
        if (
            self.cfg.train.distill
        ):  # this needs to happen BEFORE the next time-step observation is computed, to collect the "current time-step target"
        ...
        self.trajectories_with_linvel[:, 0 * 66 : 1 * 66] = (
    ...
    def post_physics_step(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(
            self._rigid_body_rot[:, 11, :], self._rigid_body_ang_vel[:, 11, :]
        )

        self.projected_gravity[:] = quat_rotate_inverse(
            self._rigid_body_rot[:, 11, :], self.gravity_vec
        )
        ...
        if self.cfg.env.im_eval:
            ...
            motion_res = self._get_state_from_motionlib_cache_trimesh(
                self.motion_ids, time, offset=offset
            )
            ...
            self.extras["mpjpe"] = (
                (diff_global_body_pos).norm(dim=-1).mean(dim=-1)
            )
    ...
    def check_termination(self):
        ...
        if (
            self.cfg.asset.terminate_by_knee_distance
            and self.knee_distance.shape
        ):
            # print("terminate_by knee_distance")
            self.reset_buf |= torch.any(
                self.knee_distance
                < self.cfg.asset.termination_scales.min_knee_distance,
                dim=1,
            )
        ...
        if self.cfg.asset.terminate_by_lin_vel:
            ...
            self.reset_buf |= torch.any(
                torch.norm(self.base_lin_vel, dim=-1, keepdim=True)
                > self.cfg.asset.termination_scales.base_vel,
                dim=1,
            )
        ...
        if self.cfg.asset.terminate_by_ang_vel:
            self.reset_buf |= torch.any(
                torch.norm(self.base_ang_vel, dim=-1, keepdim=True)
                > self.cfg.asset.termination_scales.base_ang_vel,
                dim=1,
            )
        ...
        if self.cfg.asset.terminate_by_gravity:
            ...
            self.reset_buf |= torch.any(
                ...
                > self.cfg.asset.termination_scales.gravity_x,
                dim=1,
            )
            ...
            self.reset_buf |= torch.any(
                ...
                > self.cfg.asset.termination_scales.gravity_y,
                dim=1,
            )
        ...
        if self.cfg.asset.terminate_by_low_height:
            ...
            self.reset_buf |= torch.any(
                ...
                < self.cfg.asset.termination_scales.base_height,
                dim=1,
            )
        ...
        if self.cfg.motion.teleop:
            if self.cfg.asset.terminate_by_ref_motion_distance:
                termination_distance = (
                    self.cfg.asset.termination_scales.max_ref_motion_distance
                )
                ...
                if self.cfg.env.test or self.cfg.env.im_eval:
                    reset_buf_teleop = torch.any(
                        torch.norm(
                            self._rigid_body_pos - ref_body_pos, dim=-1
                    ...
                    reset_buf_teleop = torch.any(
                        torch.norm(self._rigid_body_pos - ref_body_pos, dim=-1)
                        > termination_distance,
                        dim=-1,
                    )
            ...
            if self.cfg.asset.terminate_by_1time_motion:
                ...
                self.time_out_by_1time_motion = (
                    time > self.motion_len
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.control.action_filt:
            ...
            filter_action_ids_torch = torch.concat(
                [
                    torch.arange(
                        self.num_actions, dtype=torch.int32, device=self.device
                    )
                    + env_id * self.num_actions
        ...
        if self.cfg.motion.teleop and self.cfg.motion.curriculum:
            self.extras["episode"]["teleop_level"] = torch.mean(
                self.teleop_levels.float()
            )
    ...
    def compute_reward(self):
        ...
        for i in range(len(self.reward_functions)):
            ...
            if self.cfg.asset.zero_out_far and "teleop" in name:
                if (
                    name != "teleop_body_position_extend"
                    or name != "teleop_body_position_extend_upper_0dot5sigma"
                    or name != "teleop_body_position_extend_upper"
                    or name != "teleop_body_position_vr_3keypoints"
                ):
                    rew_ignore = (
                        ~self.prioritize_closing
                    )  # prioritize_closing = 1 if distance > close_distance, then ignore dof reward
                    rew *= rew_ignore
    ...
    def update_freeze_ref(self, motion_res, index):
        ...
        self.freeze_motion_res["rg_pos_t"][index] = motion_res["rg_pos_t"][
            index
        ]
        self.freeze_motion_res["body_vel"][index] = motion_res["body_vel"][
            index
        ]  # [num_envs, num_markers, 3]
        self.ref_body_vel = self.freeze_motion_res["body_vel"][index]
        self.freeze_motion_res["body_vel_t"][index] = motion_res["body_vel_t"][
            index
        ]  # [num_envs, num_markers, 3]
        self.freeze_motion_res["rb_rot"][index] = motion_res["rb_rot"][
            index
        ]  # [num_envs, num_markers, 4]
        self.freeze_motion_res["body_ang_vel"][index] = motion_res[
            "body_ang_vel"
        ][
            index
        ]  # [num_envs, num_markers, 3]
        self.freeze_motion_res["dof_pos"][index] = motion_res["dof_pos"][
            index
        ]  # [num_envs, num_dofs]
        self.freeze_motion_res["dof_vel"][index] = motion_res["dof_vel"][
            index
        ]  # [num_envs, num_dofs]
    ...
    def compute_self_and_task_obs(
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.future_tracks:
                ...
                time_internals = (
                    torch.arange(time_steps)
                    .to(self.device)
                    .repeat(B)
                    .view(-1, time_steps)
                    / self.cfg.motion.traj_sample_timestep_inv
                )
                motion_times_steps = (
                    (self.episode_length_buf[self.motion_ids, None] + 1)
                    * self.dt
                    + time_internals
                    + self.motion_start_times[self.motion_ids, None]
                ).flatten()  # Next frame, so +1
                env_ids_steps = self.motion_ids.repeat_interleave(time_steps)
                motion_res = self._get_state_from_motionlib_cache_trimesh(
                    env_ids_steps,
                    motion_times_steps,
                    offset=offset.repeat_interleave(time_steps, dim=0).view(
                        -1, 3
                    ),
                )
            else:
            ...
            if self.cfg.domain_rand.motion_package_loss:
                ...
                ref_body_vel_subset = self.freeze_motion_res[
                    "body_vel"
                ]  # [num_envs, num_markers, 3]
                self.ref_body_vel = ref_body_vel_subset
                ref_body_vel_extend = self.freeze_motion_res[
                    "body_vel_t"
                ]  # [num_envs, num_markers, 3]
                ref_body_rot = self.freeze_motion_res[
                    "rb_rot"
                ]  # [num_envs, num_markers, 4]
                ref_body_ang_vel = self.freeze_motion_res[
                    "body_ang_vel"
                ]  # [num_envs, num_markers, 3]
                ref_joint_pos = self.freeze_motion_res[
                    "dof_pos"
                ]  # [num_envs, num_dofs]
                ref_joint_vel = self.freeze_motion_res[
                    "dof_vel"
                ]  # [num_envs, num_dofs]
            else:
                ref_body_pos = motion_res["rg_pos"]
                ref_body_pos_extend = motion_res["rg_pos_t"]
                ref_body_vel_subset = motion_res[
                    "body_vel"
                ]  # [num_envs, num_markers, 3]
                ref_body_vel = ref_body_vel_subset
                ref_body_vel_extend = motion_res[
                    "body_vel_t"
                ]  # [num_envs, num_markers, 3]
                ref_body_rot = motion_res[
                    "rb_rot"
                ]  # [num_envs, num_markers, 4]
                ref_body_rot_extend = motion_res[
                    "rg_rot_t"
                ]  # [num_envs, num_markers, 4]
                ref_body_ang_vel = motion_res[
                    "body_ang_vel"
                ]  # [num_envs, num_markers, 3]
                ref_body_ang_vel_extend = motion_res[
                    "body_ang_vel_t"
                ]  # [num_envs, num_markers, 3]
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == "v1":
                ...
                with torch.no_grad():
                    ...
                    delta_base_pos = quat_rotate_inverse(
                        self.base_quat, delta_root_pos
                    )[:, :2]

                    forward = quat_apply(self.base_quat, self.forward_vec)
                    heading = torch.atan2(forward[:, 1], forward[:, 0])

                    ref_forward = quat_apply(
                        ref_body_rot[:, 0, :], self.forward_vec
                    )
                    ref_heading = torch.atan2(
                        ref_forward[:, 1], ref_forward[:, 0]
                    )
                    delta_heading = wrap_to_pi(ref_heading - heading).unsqueeze(
                        1
                    )
                    ...
                    ref_base_vel = quat_rotate_inverse(
                        ref_root_rot, ref_root_vel
                    )
                    ref_base_ang_vel = quat_rotate_inverse(
                        ref_root_rot, ref_root_ang_vel
                    )
                    ref_base_gravity = quat_rotate_inverse(
                        ref_root_rot, self.gravity_vec
                    )
                ...
                with torch.no_grad():
                    ...
                    task_obs = compute_imitation_observations_teleop(
                        root_pos,
                        root_rot,
                        root_vel,
                        body_pos[:, selected_keypoints_idx, :],
                        ref_body_pos[:, selected_keypoints_idx, :],
                        1,
                    )
                    ...
                    obs = torch.cat(
                        [
                            ...
                            base_gravity,
                            delta_base_pos,
                            delta_heading,  # 19dim + 19dim + 3dim + 3dim + 3dim + 2dim + 1dim
                ...
                with torch.no_grad():
                    ...
                    obs = torch.cat(
                        [
                            ...
                            base_vel,
                            base_ang_vel,
                            base_gravity,  # 19dim + 19dim + 3dim + 3dim + 3dim
                            task_obs,  # 3xselected_dim = 18dim
                            self.actions,
                        ],
                        dim=-1,
                ...
                task_obs = compute_imitation_observations_teleop(
                    ...
                    root_rot,
                    root_vel,
                    body_pos_extend[:, self._track_bodies_extend_id, :],
                    ref_rb_pos_subset,
                    1,
                )
                ...
                self.cfg.motion.teleop_obs_version == "v-teleop-extend-nolinvel"
            ):
                ...
                if self.cfg.env.add_short_history:
                    ...
                    history_to_be_append = self.trajectories_with_linvel[
                        :, 0 : self.cfg.env.short_history_length * 66
                    ]
                    obs = torch.cat(
                        [
                            dof_pos,
                ...
                self.cfg.motion.teleop_obs_version
                == "v-teleop-extend-vr-max-nolinvel-heading"
            ):
                body_pos = self._rigid_body_pos
                body_rot = self._rigid_body_rot
                body_vel = self._rigid_body_vel
                ...
                body_rot_extend = torch.cat(
                    [body_rot, body_rot[:, self.extend_body_parent_ids]], dim=1
                )
                body_rot_subset = body_rot_extend[
                    :, self._track_bodies_extend_id, :
                ]
                ...
                ref_rb_rot_subset = ref_body_rot_extend[
                    :, self._track_bodies_extend_id
                ]
                ref_body_vel_subset = ref_body_vel_extend[
                    :, self._track_bodies_extend_id
                ...
                task_obs = compute_imitation_observations_teleop_max_heading(
                    root_pos,
                    root_rot,
                    body_pos_subset,
                    body_rot_subset[:, -1, :],
                ...
                body_vel_extend = torch.cat(
                    [
                        body_vel,
                        body_vel[:, self.extend_body_parent_ids].clone(),
                    ],
                    dim=1,
                )
                body_vel_subset = body_vel_extend[
                    :, self._track_bodies_extend_id, :
                ]

                body_ang_vel_extend = torch.cat(
                    [
                        body_ang_vel,
                        body_ang_vel[:, self.extend_body_parent_ids].clone(),
                    ],
                    dim=1,
                )
                body_ang_vel_subset = body_ang_vel_extend[
                    :, self._track_bodies_extend_id, :
                ]
                ...
                ref_body_ang_vel_subset = ref_body_ang_vel_extend[
                    :, self._track_bodies_extend_id
                ]
                ...
                self_obs = compute_humanoid_observations_max_full(
                    body_pos_extend,
                    body_rot_extend,
                    body_vel_extend,
                    body_ang_vel_extend,
                ...
                task_obs = compute_imitation_observations_max_full(
                    root_pos,
                    root_rot,
                    body_pos_subset,
                    body_rot_subset,
                ...
                obs = torch.cat(
                    [
                        ...
                        dof_vel,
                        base_ang_vel,
                        base_gravity,  # 19dim + 19dim + (no 3dim) + 3dim + 3dim
                        task_obs,  #
                        self.actions,
                    ],
                    dim=-1,
                ...
                self.cfg.motion.teleop_obs_version == "v-teleop-extend-max-acc"
            ):
                body_pos = self._rigid_body_pos
                body_rot = self._rigid_body_rot
                body_vel = self._rigid_body_vel
        ...
        if self.add_noise:
            ...
            if self.cfg.motion.teleop:
                if (
                    self.cfg.motion.curriculum
                    and self.cfg.motion.obs_noise_by_curriculum
                ):
                    noise_rescale = self.teleop_levels.unsqueeze(1) / 10.0
            obs += (
        ...
        if self.cfg.env.num_privileged_obs is not None:
            ...
            self.privileged_info = torch.cat(
                [
                    ...
                    self._kd_scale,
                    self._rfi_lim_scale,
                    self.contact_forces[:, self.feet_indices, :].reshape(
                        self.num_envs, 6
                    ),
            ...
            privileged_obs_buf = torch.cat(
                [obs_buf_denoise, self.privileged_info], dim=1
            )
    ...
    def _get_heights(self, position=None, env_ids=None):
        ...
        if env_ids:
            ...
            ) + position.view(self.num_envs, -1, 3)
        else:
    ...
    def _update_realtime_vr_keypoints(self, keypoints_pos, keypoints_vel):
        ...
        self.realtime_vr_keypoints_pos = torch.Tensor(keypoints_pos).to(
            self.device
        )
        self.realtime_vr_keypoints_vel = torch.Tensor(keypoints_vel).to(
            self.device
        )
    ...
    def _draw_debug_vis(self):
        ...
        for env_id in range(self.num_envs):
            if self.cfg.motion.teleop:
                if self.cfg.motion.realtime_vr_keypoints:
                    for vr_keypoint_idx in range(
                        self.realtime_vr_keypoints_pos.shape[0]
                    ):
                        ...
                        sphere_pose = gymapi.Transform(
                            gymapi.Vec3(
                                self.realtime_vr_keypoints_pos[
                                    vr_keypoint_idx, 0
                                ],
                                self.realtime_vr_keypoints_pos[
                                    vr_keypoint_idx, 1
                    ...
                    for pos_id, pos_joint in enumerate(
                        ...
                        color_inner = (
                            (0.3, 0.3, 0.3)
                            if not self.cfg.motion.visualize_config.customize_color
                            else self.cfg.motion.visualize_config.marker_joint_colors[
                                pos_id
                                % len(
                                    self.cfg.motion.visualize_config.marker_joint_colors
                                )
                            ]
                        )
                        color_inner = tuple(color_inner)
                        ...
                        sphere_pose = gymapi.Transform(
                            gymapi.Vec3(
                                pos_joint[0], pos_joint[1], pos_joint[2]
                            ),
                            r=None,
                        )
                        gymutil.draw_lines(
    ...
    def _process_rigid_shape_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_friction:
            ...
            for s in range(len(props)):
                ...
                # import pdb; pdb.set_trace()
                self._ground_friction_values[env_id, s] += self.friction_coeffs[
                    env_id
                ].squeeze()
        return props
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_base_com:
            ...
            com_x_bias = np.random.uniform(
                self.cfg.domain_rand.base_com_range.x[0],
                self.cfg.domain_rand.base_com_range.x[1],
            )
            com_y_bias = np.random.uniform(
                self.cfg.domain_rand.base_com_range.y[0],
                self.cfg.domain_rand.base_com_range.y[1],
            )
            com_z_bias = np.random.uniform(
                self.cfg.domain_rand.base_com_range.z[0],
                self.cfg.domain_rand.base_com_range.z[1],
            )
        ...
        if self.cfg.domain_rand.randomize_link_mass:
            for i, body_name in enumerate(
                self.cfg.domain_rand.randomize_link_body_names
            ):
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.domain_rand.push_robots and (
            ...
            if (
                self.cfg.motion.curriculum
                and self.cfg.motion.push_robot_by_curriculum
            ):
                mean_teleop_level = (self.teleop_levels / 10.0).mean()
                if torch.rand(1).to("cuda") < mean_teleop_level:
                    self._push_robots()
            else:
                self._push_robots()
        if self.cfg.domain_rand.motion_package_loss and (
            self.common_step_counter
            % self.cfg.domain_rand.package_loss_interval
            == 0
        ):
            self._freeze_ref_motion()
        if self.cfg.motion.teleop and (
            self.common_step_counter
            % self.cfg.motion.resample_motions_for_envs_interval
            == 0
        ):
            ...
            if self.cfg.motion.teleop:
                ...
                if self.cfg.domain_rand.born_offset:
                    ...
                    if rand_num < self.cfg.domain_rand.born_offset_possibility:
                        randomize_distance = torch_rand_float(
                            -self.cfg.domain_rand.born_distance,
                            self.cfg.domain_rand.born_distance,
                            (len(env_ids), 2),
                            device=self.device,
                        )
                ...
                self.root_states[env_ids, 10:13] = motion_res["root_ang_vel"][
                    env_ids
                ]

                if self.cfg.domain_rand.born_heading_randomization:
                    random_angles_rad_axis = torch.zeros(
                        len(env_ids), 3, device=self.device
                    )
                    random_angles = (
                        torch.rand((len(env_ids),), device=self.device)
                        * (2 * self.cfg.domain_rand.born_heading_degree)
                        - self.cfg.domain_rand.born_heading_degree
                    )
                    ...
                    self.root_states[env_ids, 3:7] = apply_rotation_to_quat_z(
                        self.root_states[env_ids, 3:7], random_angles_rad_axis
                    )
                # import ipdb; ipdb.set_trace()
                ...
                self._rigid_body_ang_vel[env_ids] = motion_res["body_ang_vel"][
                    env_ids
                ]
            else:
                ...
                self.root_states[env_ids, 7:13].uniform_(
                    -0.5, 0.5
                )  # random base twist
        else:
            if self.cfg.motion.teleop:
    ...
    def _freeze_ref_motion(self):
        if self.cfg.motion.teleop:
            # import ipdb; ipdb.set_trace()
            package_loss_random_time = np.random.randint(
                self.cfg.domain_rand.package_loss_range[0],
                self.cfg.domain_rand.package_loss_range[1] + 1,
            )
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        if self.cfg.motion.teleop:
            # import ipdb; ipdb.set_trace()
            teleop_distance = torch.norm(
                self._rigid_body_pos[env_ids] - ref_body_pos[env_ids], dim=-1
            ).mean(
                dim=-1
            )  # shape [num_envs]
            move_up = (
    ...
    def update_average_episode_length(self, env_ids):
        ...
        ) + current_average_episode_length * (
    ...
    def _update_sigma_curriculum(self):
        ...
        if (
            self.average_episode_length
            < self.cfg.rewards.reward_position_sigma_level_up_threshold
        ):
            self.cfg.rewards.teleop_body_pos_upperbody_sigma *= (
                1 + self.cfg.rewards.level_degree
            )
        elif (
            self.average_episode_length
            > self.cfg.rewards.reward_position_sigma_level_down_threshold
        ):
            self.cfg.rewards.teleop_body_pos_upperbody_sigma *= (
                1 - self.cfg.rewards.level_degree
            )
        self.cfg.rewards.teleop_body_pos_upperbody_sigma = np.clip(
            self.cfg.rewards.teleop_body_pos_upperbody_sigma,
            self.cfg.rewards.teleop_body_pos_upperbody_sigma_range[0],
            self.cfg.rewards.teleop_body_pos_upperbody_sigma_range[1],
        )

    def _update_penalty_curriculum(self):
        if (
            self.average_episode_length
            < self.cfg.rewards.penalty_level_down_threshold
        ):
            self.cfg.rewards.penalty_scale *= 1 - self.cfg.rewards.level_degree
        elif (
            self.average_episode_length
            > self.cfg.rewards.penalty_level_up_threshold
        ):
            self.cfg.rewards.penalty_scale *= 1 + self.cfg.rewards.level_degree
        self.cfg.rewards.penalty_scale = np.clip(
            self.cfg.rewards.penalty_scale,
            self.cfg.rewards.penalty_scale_range[0],
            self.cfg.rewards.penalty_scale_range[1],
        )

    def _update_born_offset_curriculum(self):
        if (
            self.average_episode_length
            < self.cfg.domain_rand.born_offset_level_down_threshold
        ):
            self.cfg.domain_rand.born_distance *= (
                1 - self.cfg.domain_rand.level_degree
            )
        elif (
            self.average_episode_length
            > self.cfg.domain_rand.born_offset_level_up_threshold
        ):
            self.cfg.domain_rand.born_distance *= (
                1 + self.cfg.domain_rand.level_degree
            )
        ...
        self.cfg.domain_rand.born_distance = np.clip(
            self.cfg.domain_rand.born_distance,
            self.cfg.domain_rand.born_offset_range[0],
            self.cfg.domain_rand.born_offset_range[1],
        )

    def _update_born_heading_curriculum(self):
        if (
            self.average_episode_length
            < self.cfg.domain_rand.born_heading_level_down_threshold
        ):
            self.cfg.domain_rand.born_heading_degree *= (
                1 - self.cfg.domain_rand.born_heading_level_degree
            )
        elif (
            self.average_episode_length
            > self.cfg.domain_rand.born_heading_level_up_threshold
        ):
            self.cfg.domain_rand.born_heading_degree *= (
                1 + self.cfg.domain_rand.born_heading_level_degree
            )
        # import ipdb; ipdb.set_trace()
        # torch.clamp(randomize_distance,self.cfg.domain_rand.born_offset_range[0], self.cfg.domain_rand.born_offset_range[1])
        self.cfg.domain_rand.born_heading_degree = np.clip(
            self.cfg.domain_rand.born_heading_degree,
            self.cfg.domain_rand.born_heading_range[0],
            self.cfg.domain_rand.born_heading_range[1],
        )
    ...
    def _update_teleop_curriculum(self, env_ids):
        ...
        if self.cfg.motion.teleop:
            ...
            move_up = (
                episode_length_buf
                > self.cfg.motion.teleop_level_up_episode_length
            )
            move_down = (
                episode_length_buf
                < self.cfg.motion.teleop_level_down_episode_length
            )
        else:
            raise NotImplementedError
        # import ipdb; ipdb.set_trace()
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.motion.teleop:
            ...
            if self.cfg.motion.teleop_obs_version == "v1":
                ...
                noise_vec[0:root_height_obs_end_idx] = (
                ...
                body_rot_end_idx = (
                    body_pos_end_idx + self.num_actions * 6 + 6
                )  # 6x20 -> 120dim
                noise_vec[body_pos_end_idx:body_rot_end_idx] = (
                    noise_scales.body_rot
                    * noise_level
                    * self.obs_scales.body_rot
                )  # [58: 178]

                root_lin_vel_end_idx = body_rot_end_idx + 3  # 3dim
                noise_vec[body_rot_end_idx:root_lin_vel_end_idx] = (
                    noise_scales.lin_vel * noise_level * self.obs_scales.lin_vel
                )  # [178:181]

                root_ang_vel_end_idx = root_lin_vel_end_idx + 3  # 3dim
                noise_vec[root_lin_vel_end_idx:root_ang_vel_end_idx] = (
                    noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
                )  # [181:184]

                dof_pos_end_idx = (
                    root_ang_vel_end_idx + self.num_actions
                )  # 1x19 -> 19dim
                noise_vec[root_ang_vel_end_idx:dof_pos_end_idx] = (
                    noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
                )  # [184:203]

                dof_vel_end_idx = (
                    dof_pos_end_idx + self.num_actions
                )  # 1x19 -> 19dim
                noise_vec[dof_pos_end_idx:dof_vel_end_idx] = (
                    noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
                )  # [203:222]

                # TAKS OBSERVATION
                diff_local_body_pos_flat_end_idx = (
                    dof_vel_end_idx + self.num_actions * 3 + 3
                )  # 3x20 -> 60dim
                noise_vec[dof_vel_end_idx:diff_local_body_pos_flat_end_idx] = (
                    noise_scales.ref_body_pos
                    * noise_level
                    * self.obs_scales.body_pos
                )  # [222:282]

                diff_local_body_rot_flat_end_idx = (
                    diff_local_body_pos_flat_end_idx + self.num_actions * 6 + 6
                )  # 6x20 -> 120dim
                noise_vec[
                    diff_local_body_pos_flat_end_idx:diff_local_body_rot_flat_end_idx
                ] = (
                    noise_scales.ref_body_rot
                    * noise_level
                    * self.obs_scales.body_rot
                )  # [282:402]

                diff_local_lin_vel_end_idx = (
                    diff_local_body_rot_flat_end_idx + 3
                )  # 3dim
                noise_vec[
                    diff_local_body_rot_flat_end_idx:diff_local_lin_vel_end_idx
                ] = (
                    noise_scales.ref_lin_vel
                    * noise_level
                    * self.obs_scales.lin_vel
                )  # [402:405]

                diff_local_ang_vel_end_idx = (
                    diff_local_lin_vel_end_idx + 3
                )  # 3dim
                noise_vec[
                    diff_local_lin_vel_end_idx:diff_local_ang_vel_end_idx
                ] = (
                    noise_scales.ref_ang_vel
                    * noise_level
                    * self.obs_scales.ang_vel
                )  # [405:408]

                diff_local_ref_body_pos_root_end_idx = (
                    diff_local_ang_vel_end_idx + self.num_actions * 3 + 3
                )  # 3x20 -> 60dim
                noise_vec[
                    diff_local_ang_vel_end_idx:diff_local_ref_body_pos_root_end_idx
                ] = (
                    noise_scales.ref_body_pos
                    * noise_level
                    * self.obs_scales.body_pos
                ...
                diff_local_ref_body_rot_root_end_idx = (
                    diff_local_ref_body_pos_root_end_idx
                    + self.num_actions * 6
                    + 6
                )  # 6x20 -> 120dim
                noise_vec[
                    diff_local_ref_body_pos_root_end_idx:diff_local_ref_body_rot_root_end_idx
                ] = (
                    noise_scales.ref_body_rot
                    * noise_level
                    * self.obs_scales.body_rot
                ...
                diff_dof_pos_end_idx = (
                    diff_local_ref_body_rot_root_end_idx + self.num_actions
                )  # 1x19 -> 19dim
                noise_vec[
                    diff_local_ref_body_rot_root_end_idx:diff_dof_pos_end_idx
                ] = (
                    noise_scales.ref_dof_pos
                    * noise_level
                    * self.obs_scales.dof_pos
                )  # [588:607]

                diff_dof_vel_end_idx = (
                    diff_dof_pos_end_idx + self.num_actions
                )  # 1x19 -> 19dim
                noise_vec[diff_dof_pos_end_idx:diff_dof_vel_end_idx] = (
                    noise_scales.ref_dof_vel
                    * noise_level
                ...
                noise_vec[diff_dof_vel_end_idx:projected_gravity_end_idx] = (
                    noise_scales.gravity * noise_level
                )  # [626:629]
                # import ipdb; ipdb.set_trace()
                # LAST ACTION
                last_action_end_idx = (
                    projected_gravity_end_idx + self.num_actions
                )
                noise_vec[projected_gravity_end_idx:last_action_end_idx] = (
                    0.0  # [629:648]
                )
            elif self.cfg.motion.teleop_obs_version == "v-min":
                ...
                # base gravity
                noise_vec[2 * self.num_dof + 6 : 2 * self.num_dof + 9] = (
                    noise_scales.gravity * noise_level
                )
                # x y heading targets
                noise_vec[2 * self.num_dof + 9 : 2 * self.num_dof + 12] = 0.0

                # ref dof pos
                noise_vec[2 * self.num_dof + 12 : 3 * self.num_dof + 12] = (
                    noise_scales.ref_dof_pos
                    * noise_level
                    * self.obs_scales.dof_pos
                )
                # ref dof vel
                noise_vec[3 * self.num_dof + 12 : 4 * self.num_dof + 12] = (
                    noise_scales.ref_dof_vel
                    * noise_level
                    * self.obs_scales.dof_vel
                )
                # ref base vel
                noise_vec[4 * self.num_dof + 12 : 4 * self.num_dof + 15] = (
                    noise_scales.ref_lin_vel
                    * noise_level
                    * self.obs_scales.lin_vel
                )
                # ref base ang vel
                noise_vec[4 * self.num_dof + 15 : 4 * self.num_dof + 18] = (
                    noise_scales.ref_ang_vel
                    * noise_level
                    * self.obs_scales.ang_vel
                )
                # ref base gravity
                noise_vec[4 * self.num_dof + 18 : 4 * self.num_dof + 21] = (
                    noise_scales.ref_gravity * noise_level
                )

                # self.actions
                noise_vec[4 * self.num_dof + 21 :] = 0.0
            elif self.cfg.motion.teleop_obs_version == "v-min2":
                ...
                # delta base pos dim2
                noise_vec[2 * self.num_dof + 9 : 2 * self.num_dof + 11] = (
                    noise_scales.delta_base_pos
                    * noise_level
                    * self.obs_scales.delta_base_pos
                )
                # delta heading dim=1
                noise_vec[2 * self.num_dof + 11 : 2 * self.num_dof + 12] = (
                    noise_scales.delta_heading
                    * noise_level
                    * self.obs_scales.delta_heading
                )
                # ref dof pos
                noise_vec[2 * self.num_dof + 12 : 3 * self.num_dof + 12] = (
                    noise_scales.ref_dof_pos
                ...
                # last actions
                noise_vec[3 * self.num_dof + 12 : 4 * self.num_dof + 12] = (
                    noise_scales.last_action * noise_level
                )
            elif self.cfg.motion.teleop_obs_version == "v-teleop":
                ...
                noise_vec[
                    ...
                    + 12 : 2 * self.num_dof
                    + 12
                    + len(self.cfg.motion.teleop_selected_keypoints_names) * 3
                ] = (
                    noise_scales.ref_body_pos
                    * noise_level
                    * self.obs_scales.body_pos
                ...
                noise_vec[
                    ...
                    + 9 : 2 * self.num_dof
                    + 9
                    + (len(self.cfg.motion.teleop_selected_keypoints_names) + 2)
                    * 3
                ] = (
                    noise_scales.ref_body_pos
                    * noise_level
                ...
                # base ang vel
                noise_vec[
                    2 * self.num_dof + 3 - 3 : 2 * self.num_dof + 6 - 3
                ] = (
                    noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
                )
                # base gravity
                noise_vec[
                    2 * self.num_dof + 6 - 3 : 2 * self.num_dof + 9 - 3
                ] = (noise_scales.gravity * noise_level)
                # ref dof pos
                noise_vec[
                    2 * self.num_dof
                ...
                # base vel
                noise_vec[2 * self.num_dof : 2 * self.num_dof + 3] = (
                    noise_scales.lin_acc * noise_level * self.obs_scales.lin_acc
                )  # need to modify
                # base ang vel
                noise_vec[2 * self.num_dof + 3 : 2 * self.num_dof + 6] = (
                    noise_scales.ang_vel * noise_level * self.obs_scales.ang_vel
    ...
    def _episodic_domain_randomization(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_pd_gain:
            ...
            if (
                self.cfg.motion.curriculum
                and self.cfg.motion.kpkd_by_curriculum
        ...
        if self.cfg.domain_rand.randomize_rfi_lim:
            self._rfi_lim_scale[env_ids] = torch_rand_float(
                self.cfg.domain_rand.rfi_lim_range[0],
                self.cfg.domain_rand.rfi_lim_range[1],
                (len(env_ids), self.num_actions),
                device=self.device,
            )

            if self.cfg.motion.curriculum and self.cfg.motion.rfi_by_curriculum:
                rfi_lim_scale_offset_from_lowerlimit = (
                    self._rfi_lim_scale[env_ids]
                    - self.cfg.domain_rand.rfi_lim_range[0]
                )
                self._rfi_lim_scale[
                    env_ids
        ...
        if self.cfg.domain_rand.randomize_motion_ref_xyz:
            ...
            self.ref_episodic_offset[env_ids, 0] = torch_rand_float(
                self.cfg.domain_rand.motion_ref_xyz_range[0][0],
                self.cfg.domain_rand.motion_ref_xyz_range[0][1],
                (len(env_ids), 1),
                device=self.device,
            ).squeeze(1)
            self.ref_episodic_offset[env_ids, 1] = torch_rand_float(
                self.cfg.domain_rand.motion_ref_xyz_range[1][0],
                self.cfg.domain_rand.motion_ref_xyz_range[1][1],
                (len(env_ids), 1),
                device=self.device,
            ).squeeze(1)
            self.ref_episodic_offset[env_ids, 2] = torch_rand_float(
                self.cfg.domain_rand.motion_ref_xyz_range[2][0],
                self.cfg.domain_rand.motion_ref_xyz_range[2][1],
                (len(env_ids), 1),
                device=self.device,
            ).squeeze(1)
    ...
    def _init_buffers(self):
        ...
        self.last_base_lin_vel = torch.zeros_like(
            self.base_lin_vel
        )  # different from self.last_root_vel
        ...
        if self.cfg.motion.teleop:
            ...
            self.marker_coords = torch.zeros(
                self.num_envs,
                (self.num_dofs + (4 if self.cfg.motion.extend_head else 3))
                * self.cfg.motion.num_traj_samples,
                3,
                dtype=torch.float,
                device=self.device,
            ...
            if self.cfg.motion.curriculum:
                self.teleop_levels = torch.randint(
                    0, 10 + 1, (self.num_envs,), device=self.device
                )
    ...
    def _init_domain_params(self):
        ...
        self._link_mass_scale = torch.ones(
            self.num_envs,
            len(self.cfg.domain_rand.randomize_link_body_names),
            dtype=torch.float,
            device=self.device,
            requires_grad=False,
        )
    ...
    def _create_envs(self):
        ...
        if (
            (not self.headless)
            and self.cfg.motion.visualize
            and self.cfg.motion.teleop
        ):
            self._num_teleop_markers = (
                self.cfg.motion.num_markers * self.num_envs
            )
        ...
        for i in range(self.num_envs):
            ...
            if (
                ...
                assert (
                    self.cfg.motion.num_markers == self.num_dofs
                ), "we visualize all joints"
    ...
    def _load_motion(self):
        motion_path = self.cfg.motion.motion_file.format(
            LEGGED_GYM_ROOT_DIR=LEGGED_GYM_ROOT_DIR
        )
        skeleton_path = self.cfg.motion.skeleton_file.format(
            LEGGED_GYM_ROOT_DIR=LEGGED_GYM_ROOT_DIR
        )
    ...
    def _get_state_from_motionlib_cache(
    ...
    def _get_state_from_motionlib_cache_trimesh(
        self, motion_ids, motion_times, offset=None
    ):
        ## Cache the motion + offset
        # import ipdb; ipdb.set_trace()
        ...
        if self.cfg.terrain.measure_heights:
            self.measured_heights = self._get_heights(
                position=motion_res["root_pos"][:, :3]
            ).flatten()
    ...
    def _load_marker_asset(self):
        asset_path = self.cfg.motion.marker_file.format(
            LEGGED_GYM_ROOT_DIR=LEGGED_GYM_ROOT_DIR
        )
        asset_root = os.path.dirname(asset_path)
        asset_file = os.path.basename(asset_path)
        ...
        self._marker_asset = self.gym.load_asset(
            self.sim, asset_root, asset_file, marker_asset_options
        )
        return

    # ------------ helper functions ---------------
    def _get_rigid_body_pos(self, body_name):
        body_list = self.gym.get_actor_rigid_body_names(
        ...
        )[body_list.index(body_name) :: len(body_list), :3]
    ...
    def knee_distance(self):
        ...
        dist_knee = torch.norm(
            left_knee_pos - right_knee_pos, dim=-1, keepdim=True
        )
    ...
    def feet_distance(self):
        ...
        dist_feet = torch.norm(
            left_foot_pos - right_foot_pos, dim=-1, keepdim=True
        )
        return dist_feet
    ...
    def _reward_stable_lower_when_vrclose(self):
        assert (
            self.cfg.motion.extend_head
        ), "This reward is only for the extended model"
        ...
        far_enough = (
            ...
            > self.cfg.rewards.vrclose_threshold
        )
        ...
        reward *= (
            torch.norm(ref_body_vel[:, 0, :2], dim=1)
            < self.cfg.rewards.ref_stable_velocity_threshold
        )  # no reward for low ref motion velocity (root xy velocity)
        # import ipdb; ipdb.set_trace()
        return reward
    ...
    def _reward_base_height(self):
        ...
        return torch.clip(dif - 0.15, min=0.0)
    ...
    def _reward_lower_action_rate(self):
        # Penalize changes in actions
        return torch.sum(
            torch.square(self.last_actions[:, :11] - self.actions[:, :11]),
            dim=1,
        )
    ...
    def _reward_freeze_upper_body(self):
        return torch.mean(
            torch.square(self.dof_pos[:, 10:] - self.default_dof_pos[:, 10:]),
            dim=1,
        )
    ...
    def _reward_tracking_dof_vel(self):
        ...
        dof_vel_error = torch.sum(
            torch.square(self.joint_vel_reference - self.dof_vel), dim=1
        )
    ...
    def _reward_teleop_selected_joint_position(self):
        ...
        for (
            joint_name,
            scale,
        ) in self.cfg.rewards.teleop_joint_pos_selection.items():
            joint_index = self.dof_names.index(joint_name)
            assert (
                joint_index >= 0
            ), f"Joint {joint_name} not found in the robot"

            diff_dof_pos[:, joint_index] *= scale**0.5
        diff_dof_pos_dist = torch.mean(torch.square(diff_dof_pos), dim=1)
        r_dof_pos = torch.exp(
            -diff_dof_pos_dist / self.cfg.rewards.teleop_joint_pos_sigma
        )
    ...
    def _reward_teleop_joint_vel_lower(self):
        ...
        r_dof_vel = torch.exp(
            -diff_dof_vel_dist / self.cfg.rewards.teleop_joint_vel_sigma
        )
        return r_dof_vel
    ...
    def _reward_teleop_selected_joint_vel(self):
        ...
        for (
            ...
            diff_dof_vel[:, joint_index] *= scale**0.5
        diff_dof_vel_dist = torch.mean(torch.square(diff_dof_vel), dim=1)
        r_dof_vel = torch.exp(
            -diff_dof_vel_dist / self.cfg.rewards.teleop_joint_vel_sigma
        )
    ...
    def _reward_teleop_body_position(self):
        ...
        r_body_pos = torch.exp(
            -diff_body_pos_dist / self.cfg.rewards.teleop_body_pos_sigma
        )
        return r_body_pos
    ...
    def _reward_teleop_body_position_extend_small_sigma(self):
        ...
        diff_body_pos_dist = (diff_global_body_pos**2).mean(dim=-1).mean(dim=-1)
        r_body_pos = torch.exp(
            -diff_body_pos_dist / self.cfg.rewards.teleop_body_pos_small_sigma
        )
    ...
    def _reward_teleop_body_position_extend(self):
        ...
        diff_body_pos_dist_lower = (
            (diff_global_body_pos_lower**2).mean(dim=-1).mean(dim=-1)
        )
        diff_body_pos_dist_upper = (
            (diff_global_body_pos_upper**2).mean(dim=-1).mean(dim=-1)
        )
        ...
        r_body_pos_lower = torch.exp(
            -diff_body_pos_dist_lower
            / self.cfg.rewards.teleop_body_pos_lowerbody_sigma
        )
        ...
        r_body_pos = (
            r_body_pos_lower * self.cfg.rewards.teleop_body_pos_lowerbody_weight
            + r_body_pos_upper
            * self.cfg.rewards.teleop_body_pos_upperbody_weight
        )
    ...
    def _reward_teleop_body_position_extend_upper_0dot5sigma(self):
        ...
        r_body_pos_upper = torch.exp(
            -diff_body_pos_dist_upper
            / self.cfg.rewards.teleop_body_pos_0dot5sigma
        )
    ...
    def _reward_teleop_body_position_vr_3keypoints(self):
        ...
        diff_body_pos_dist_vr_3keypoints = (
            (diff_global_body_pos_vr_3keypoints**2).mean(dim=-1).mean(dim=-1)
        )

        diff_body_pos_dist_vr_3keypoints = diff_body_pos_dist_vr_3keypoints

        r_body_pos_vr_3keypoints = torch.exp(
            -diff_body_pos_dist_vr_3keypoints
            / self.cfg.rewards.teleop_body_pos_vr_3keypoints_sigma
        )
    ...
    def _reward_teleop_body_rotation(self):  # wholebody
        ...
        diff_global_body_angle = torch_utils.quat_to_angle_axis(
            diff_global_body_rot
        )[0]
        diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
        r_body_rot = torch.exp(
            -diff_global_body_angle_dist
            / self.cfg.rewards.teleop_body_rot_sigma
        )
        return r_body_rot
    ...
    def _reward_teleop_selected_body_rotation(self):
        ...
        track_rot_body_indices = [
            self._body_list.index(body_name)
            for body_name in self.cfg.rewards.teleop_body_rot_selection
        ]
        diff_global_body_angle_dist = (
            diff_global_body_angle[:, track_rot_body_indices] ** 2
        ).mean(dim=-1)
        r_body_rot = torch.exp(
            -diff_global_body_angle_dist
            / self.cfg.rewards.teleop_body_rot_sigma
    ...
    def _reward_teleop_body_vel(self):
        ...
        r_vel = torch.exp(
            -diff_global_vel_dist / self.cfg.rewards.teleop_body_vel_sigma
        )
        return r_vel
    ...
    def _reward_teleop_selected_body_vel(self):
        ...
        track_vel_body_indices = [
            self._body_list.index(body_name)
            for body_name in self.cfg.rewards.teleop_body_vel_selection
        ]
        diff_global_vel_dist = (
            (diff_global_vel[:, track_vel_body_indices] ** 2)
            .mean(dim=-1)
            .mean(dim=-1)
        )
        r_vel = torch.exp(
    ...
    def _reward_teleop_body_ang_vel(self):
        ...
        diff_global_ang_vel_dist = (
            (diff_global_ang_vel**2).mean(dim=-1).mean(dim=-1)
        )
        r_ang_vel = torch.exp(
            -diff_global_ang_vel_dist
            / self.cfg.rewards.teleop_body_ang_vel_sigma
        )
        return r_ang_vel
    ...
    def _reward_teleop_selected_body_ang_vel(self):
        ...
        track_ang_vel_body_indices = [
            self._body_list.index(body_name)
            for body_name in self.cfg.rewards.teleop_body_ang_vel_selection
        ]
        diff_global_ang_vel_dist = (
            (diff_global_ang_vel[:, track_ang_vel_body_indices] ** 2)
            .mean(dim=-1)
            .mean(dim=-1)
        )
        r_ang_vel = torch.exp(
    ...
    def _reward_feet_max_height_for_this_air(self):
        ...
        self.feet_air_max_height = torch.max(
            ...
            self._rigid_body_pos[:, self.feet_indices, 2],
        )
    ...
    def _reward_move_or_not(self):
        should_move = torch.logical_or(
            torch.norm(self.commands[:, :2], dim=1) > 0.1,
            self.commands[:, 2] > 0.2,
        )
        is_moving = self.dof_vel.norm(dim=1) > 0.2
        return (2.0 * is_moving - 1) * (2.0 * should_move - 1.0)
    ...
    def _reward_freeze_arms(self):
        ...
        return torch.sum(
            torch.square(self.dof_pos[:, 11:] - self.default_dof_pos[:, 11:]),
            dim=1,
        ) + 0.1 * torch.abs(self.dof_pos[:, 10] - self.default_dof_pos[:, 10])
    ...
    def _update_package_loss_count(self):
        ...
        self._package_loss_counter = torch.clamp_min(
            self._package_loss_counter, 0
        )
        return
...
def compute_imitation_observations(
    ...
    diff_global_body_pos = ref_body_pos.view(
        B, time_steps, J, 3
    ) - body_pos.view(B, 1, J, 3)
    ...
    diff_global_body_rot = torch_utils.quat_mul(
        ...
        torch_utils.quat_conjugate(
            body_rot[:, None].repeat_interleave(time_steps, 1)
        ),
    )
    diff_local_body_rot_flat = torch_utils.quat_mul(
        torch_utils.quat_mul(
            heading_inv_rot_expand.view(-1, 4), diff_global_body_rot.view(-1, 4)
        ),
        heading_rot_expand.view(-1, 4),
    )  # Need to be change of basis

    ##### linear and angular  Velocity differences for root
    diff_global_root_vel = ref_root_vel.view(B, time_steps, 3) - root_vel.view(
        B, 1, 3
    )
    diff_local_root_vel = torch_utils.my_quat_rotate(
        heading_inv_rot.view(-1, 4), diff_global_root_vel.view(-1, 3)
    )

    diff_global_root_ang_vel = ref_root_ang_vel.view(
        B, time_steps, 3
    ) - root_ang_vel.view(B, 1, 3)
    diff_local_root_ang_vel = torch_utils.my_quat_rotate(
        heading_inv_rot.view(-1, 4), diff_global_root_ang_vel.view(-1, 3)
    )
    ...
    local_ref_body_rot = torch_utils.quat_mul(
        heading_inv_rot_expand.view(-1, 4), ref_body_rot.view(-1, 4)
    )
    ...
    obs.append(
        diff_local_body_pos_flat.view(B, time_steps, -1)
    )  # 1 * timestep * J * 3
    obs.append(
        torch_utils.quat_to_tan_norm(diff_local_body_rot_flat).view(
            B, time_steps, -1
        )
    )  #  1 * timestep * J * 6
    obs.append(diff_local_root_vel.view(B, time_steps, -1))  # timestep  * J * 3
    obs.append(
        diff_local_root_ang_vel.view(B, time_steps, -1)
    )  # timestep  * J * 3
...
# @torch.jit.script
def compute_imitation_observations_teleop(
    root_pos, root_rot, root_vel, body_pos, ref_body_pos, time_steps
):
...
def compute_imitation_observations_teleop_max(
    ...
    local_ref_body_vel = torch_utils.my_quat_rotate(
        heading_inv_rot_expand.view(-1, 4), ref_body_vel.view(-1, 3)
    )

    if ref_episodic_offset is not None:
        # import ipdb; ipdb.set_trace()
        diff_global_body_pos_offset = (
            ref_episodic_offset.unsqueeze(1).unsqueeze(2).expand(-1, 1, J, -1)
        )
        ...
        diff_local_body_pos_flat = (
            diff_local_body_pos_flat.view(B, 1, J, 3)
            + diff_global_body_pos_offset
        )
        local_ref_body_pos_offset = ref_episodic_offset.repeat(J, 1)[
            : J * ref_episodic_offset.shape[0], :
        ]
        local_ref_body_pos[2::3] += local_ref_body_pos_offset.repeat_interleave(
            time_steps, 0
        )[2::3]
    ...
    if ref_vel_in_task_obs:
        obs.append(
            local_ref_body_vel.view(B, time_steps, -1)
        )  # timestep  * J * 3
...
# @torch.jit.script
def compute_imitation_observations_teleop_max_heading(
    root_pos,
    root_rot,
    body_pos,
    head_rot,
    ...
    ###### Heading rot
    diff_global_body_rot = torch_utils.quat_mul(
        ref_head_rot.view(B, time_steps, J, 4),
        torch_utils.quat_conjugate(
            head_rot[:, None].repeat_interleave(time_steps, 1)
        ),
    )
    diff_local_body_rot_flat = torch_utils.quat_mul(
        torch_utils.quat_mul(
    ...
    diff_local_heading_rot_flat = torch_utils.calc_heading(
    ...
    obs.append(local_ref_body_pos.view(B, time_steps, -1))  # timestep  * J * 3
    obs.append(
        diff_local_heading_rot_flat.view(B, time_steps, -1)
    )  # 1 * timestep * J * 3
    if ref_vel_in_task_obs:
        obs.append(
            local_ref_body_vel.view(B, time_steps, -1)
...
def compute_humanoid_observations(
    ...
    flat_expanded_heading_rot_inv = heading_rot_inv_expand.reshape(
    ...
    flat_local_body_pos = torch_utils.my_quat_rotate(
        flat_expanded_heading_rot_inv, flat_local_body_pos
    )
    ...
    local_body_vel = torch_utils.my_quat_rotate(
        heading_rot_inv, root_vel
    )  #  1x3, root
    local_body_ang_vel = torch_utils.my_quat_rotate(
        heading_rot_inv, root_ang_vel
    )  # 1x3, root
...
# @torch.jit.script
def compute_humanoid_observations_max_full(
...
# @torch.jit.script
def compute_imitation_observations_max_full(
    root_pos,
    root_rot,
    body_pos,
    body_rot,
    ...
    diff_global_vel = ref_body_vel.view(B, time_steps, J, 3) - body_vel.view(
        B, 1, J, 3
    )
    ...
    diff_global_ang_vel = ref_body_ang_vel.view(
        B, time_steps, J, 3
    ) - body_ang_vel.view(B, 1, J, 3)
    ...
    if ref_episodic_offset is not None:
        ...
        local_ref_body_pos += local_ref_body_pos_offset.repeat_interleave(
            time_steps, 0
        )
