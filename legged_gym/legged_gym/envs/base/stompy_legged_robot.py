...
class StompyLeggedRobot(BaseTask):
    def __init__(
        self,
        cfg: LeggedRobotCfg,
        sim_params,
        ...
        self.trajectories = torch.zeros(self.num_envs, 60 * 100).to(
            self.device
    ...
    def post_physics_step(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(
            self._rigid_body_rot[:, 0, :], self._rigid_body_ang_vel[:, 0, :]
        )

        self.projected_gravity[:] = quat_rotate_inverse(
            self._rigid_body_rot[:, 0, :], self.gravity_vec
        )
    ...
    def begin_seq_motion_samples(self):
        ...
        self._motion_lib.load_motions(
            ...
            gender_betas=[torch.zeros(18)] * self.num_envs,
    ...
    def compute_self_and_task_obs(self):
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == "v-teleop-extend-max-full":
                ...
                body_pos_extend = torch.cat(
                    [body_pos, extend_curr_pos], dim=1
                )  # [N, 22, 3]
                ...
                # 22 * 3 + 22 * 4 + 22 * 3 + 22 * 3  = 286 | pos, rot, vel, ang_vel
                ...
                # 22 * 3 + 22 * 4 + 22 * 3 + 22 * 3 + 22 * 3 + 22* 6  = 552 diff pos, rot, vel, ang_vel | pos, rot
    ...
    def _load_motion(self):
        ...
        self._motion_lib = MotionLibStompy(
    ...
    def _get_state_from_motionlib_cache_trimesh(
        ...
        motion_res["rg_pos"][..., 2] -= (
            motion_res["rg_pos"][..., 2].min().item()
        )  # Make the motion grounded. Atleast one frame from the video should have a body part touching the ground.
        ...
        if self.cfg.terrain.measure_heights:
            ...
            if "rg_pos_t" in motion_res:
                motion_res["rg_pos_t"][:, :, 2] += delta_height.unsqueeze(
                    1
                )  # rg_pos_t is not being used anywhere for stompy. verify and remove
    ...
    def _reward_lower_action_rate(self):
        ...
        return torch.sum(
            torch.square(
                self.last_actions[:, self.lower_body_indices]
                - self.actions[:, self.lower_body_indices]
            ),
            dim=1,
        )
    ...
    def _reward_upper_action_rate(self):
        ...
        return torch.sum(
            torch.square(
                self.last_actions[:, self.upper_body_indices]
                - self.actions[:, self.upper_body_indices]
            ),
            dim=1,
        )
    ...
    def _reward_close_feet(self):
        ...
        return (self.feet_distance < 0.2).squeeze(-1) * 1.0
    ...
    def _reward_freeze_upper_body(self):
        return torch.mean(
            torch.square(
                self.dof_pos[:, self.upper_body_indices]
                - self.default_dof_pos[:, self.upper_body_indices]
            ),
            dim=1,
        )
    ...
    def _reward_teleop_joint_position_lower(self):
        ...
        diff_dof_pos = diff_dof_pos[
            :, self.lower_body_indices
        ]  # lower, not including torso
    ...
    def _reward_teleop_body_position_extend(self):
        ...
        diff_global_body_pos = ref_body_pos_extend - body_pos_extend
        diff_global_body_pos_lower = diff_global_body_pos[
            :, self.lower_body_indices
        ]
        diff_global_body_pos_upper = diff_global_body_pos[
    ...
    def _reward_teleop_body_rotation_lower(self):
        ...
        diff_global_body_angle = diff_global_body_angle[
            :, self.lower_body_indices
        ]  # lower
        diff_global_body_angle_dist = (diff_global_body_angle**2).mean(dim=-1)
        r_body_rot = torch.exp(
    ...
    def _reward_teleop_body_ang_vel_lower(self):
        ...
        diff_global_ang_vel = ref_body_ang_vel - body_ang_vel
        diff_global_ang_vel = diff_global_ang_vel[
            :, self.lower_body_indices
        ]  # lower
        diff_global_ang_vel_dist = (
            (diff_global_ang_vel**2).mean(dim=-1).mean(dim=-1)
    ...
    def _reward_slippage(self):
        assert self._rigid_body_vel.shape[1] == 19
    ...
    def _reward_freeze_arms(self):
        ...
        return torch.sum(
            ...
            dim=1,
        ) + 0.1 * torch.abs(self.dof_pos[:, 9] - self.default_dof_pos[:, 9])
