...
class StompyLeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.trajectories = torch.zeros(self.num_envs, 60 * 100).to(self.device) # 18dof + 18dofvel + 3angular velocity + 3projectedgravity + 18lastaction
        self.trajectories_with_linvel = torch.zeros(self.num_envs, 63 * 100).to(self.device) # 18dof + 18dofvel + 3angular velocity + 3projectedgravity + 18lastaction
        if self.cfg.train_velocity_estimation:
            # self.velocity_estimator = VelocityEstimator(63, 512, 256, 3, 25).to(self.device)
            self.velocity_estimator = VelocityEstimatorGRU(60, 512, 3).to(self.device)
        ...
        if self.cfg.use_velocity_estimation:
            ...
            self.velocity_estimator = VelocityEstimator(60, 512, 256, 3, 25).to(self.device)
    ...
    def post_physics_step(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(self._rigid_body_rot[:, 0, :], self._rigid_body_ang_vel[:, 0, :])
        

        self.projected_gravity[:] = quat_rotate_inverse(self._rigid_body_rot[:, 0, :], self.gravity_vec)
    ...
    def compute_self_and_task_obs(self, ):
        ...
        if self.cfg.motion.teleop:
            if self.cfg.motion.teleop_obs_version == 'v1':
                ...
                extend_curr_pos = torch_utils.my_quat_rotate(body_rot[:, self.extend_body_parent_ids].reshape(-1, 4), self.extend_body_pos[:, ].reshape(-1, 3)).view(self.num_envs, -1, 3) + body_pos[:, self.extend_body_parent_ids] # [N, 3, 3]
                ...
                self_obs = compute_humanoid_observations_max_full(body_pos_extend, body_rot_extend, body_vel_extend, body_ang_vel_extend, True, False)
                ...
                obs = torch.cat([ self_obs, 
                                            task_obs,  # 
                                            self.actions], dim = -1) # 327 + 216 + 18 = 561
