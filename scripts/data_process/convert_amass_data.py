...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default="data/amass_db_smplh.pt")
    ...
    all_pkls = glob.glob("data/AMASS/**/*.npz", recursive=True)
    ...
    for data_path in tqdm(all_pkls):
        ...
        smpl_local_robot.write_xml(f"phc/phc/data/assets/mjcf/{robot_cfg['model']}_humanoid.xml")
        skeleton_tree = SkeletonTree.from_mjcf(f"phc/phc/data/assets/mjcf/{robot_cfg['model']}_humanoid.xml")
    ...
    if upright_start:
        joblib.dump(amass_full_motion_dict, "data/amass_train_take6_upright.pkl", compress=True)
    else:
        joblib.dump(amass_full_motion_dict, "data/amass_train_take6.pkl", compress=True)
