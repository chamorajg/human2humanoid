...
stompy_joint_names_augment = stompy_joint_names + [
    "left_hand_keypoint_link",
    "right_hand_keypoint_link",
    "head_link",
]
stompy_joint_pick = [
    "L_thigh",
...
stompy_joint_pick_idx = [
    stompy_joint_names_augment.index(j) for j in stompy_joint_pick
]
...
amass_data = joblib.load(
    os.path.join(base_dir, "PHC/sample_data/amass_copycat_take6_train.pkl")
)  # From PHC
...
for data_key in pbar:
    ...
    pose_aa_walk = (
        torch.from_numpy(
            np.concatenate(
                (amass_data[data_key]["pose_aa"][:, :66], np.zeros((N, 6))),
                axis=-1,
            )
        )
        .float()
    ...
    for iteration in range(500):
        ...
        pose_aa_stompy_new = torch.cat(
            [
                gt_root_rot[None, :, None],
                stompy_rotation_axis * dof_pos_new,
                torch.zeros((1, N, 2, 3)).to(device),
            ],
            axis=2,
        ).to(device)
        fk_return = stompy_fk.fk_batch(
            pose_aa_stompy_new, root_trans_offset[None,], remove_extension=False
        )
        diff = (
            fk_return["global_translation"][:, :, stompy_joint_pick_idx]
        ...
        dof_pos_new.data.clamp_(
            stompy_fk.joints_range[:, 0, None],
            stompy_fk.joints_range[:, 1, None],
        )

    dof_pos_new.data.clamp_(
        stompy_fk.joints_range[:, 0, None], stompy_fk.joints_range[:, 1, None]
    )
    pose_aa_stompy_new = torch.cat(
        [
            gt_root_rot[None, :, None],
    ...
    root_trans_offset_dump[..., 2] -= (
        fk_return.global_translation[..., 2].min().item() + STOMPY_TORSO_OFFSET
    )
    data_dump[data_key] = {
