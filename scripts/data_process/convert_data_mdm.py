...
for i in range(len(res_data["json_file"]["thetas"])):
    ...
    pose_aa = (
        sRot.from_euler("XYZ", pose_euler.reshape(-1, 3), degrees=True)
        .as_rotvec()
        .reshape(B, 72)
    )
...
for key_name in tqdm(amass_data.keys()):
    ...
    beta = (
        smpl_data_entry["beta"].copy()
        if "beta" in smpl_data_entry
        else smpl_data_entry["betas"].copy()
    )
    if len(beta.shape) == 2:
        beta = beta[0]
    ...
    skeleton_tree = SkeletonTree.from_mjcf(
        "egoquest/data/assets/mjcf/smpl_humanoid_1.xml"
    )
    ...
    if robot_cfg["upright_start"]:
        ...
        root_trans_offset = filters.gaussian_filter1d(
            root_trans_offset, 3, axis=0, mode="nearest"
        )
        root_trans_offset = torch.from_numpy(root_trans_offset)
        pose_quat_global = np.stack(
            [
                quat_correct(pose_quat_global[:, i])
                for i in range(pose_quat_global.shape[1])
            ],
            axis=1,
        )
        ...
        filtered_quats = filters.gaussian_filter1d(
            pose_quat_global, 2, axis=0, mode="nearest"
        )
        pose_quat_global = (
            filtered_quats / np.linalg.norm(filtered_quats, axis=-1)[..., None]
        )
        print("############### filtering!!! ###############")
        new_sk_state = SkeletonState.from_rotation_and_root_translation(
            skeleton_tree,
