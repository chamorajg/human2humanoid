...
class MotionLibStompy(MotionLibBase):
    ...
    def __init__(
        ...
        extend_hand=True,
        extend_head=False,
        mjcf_file="legged_gym/legged_gym/resources/robots/stompypro/robot_test.xml",
    ):
        super().__init__(
        ...
        self.mesh_parsers = Stompy_Batch(
    ...
    def load_motions(
        ...
        if "global_translation_extend" in motions[0].__dict__:
            ...
            self.gavs_t = (
                torch.cat(
                    [m.global_angular_velocity_extend for m in motions], dim=0
                )
                .float()
                .to(self._device)
            )
        ...
        min_value = min(
            self.gts_t[:, 5, 2].min().item(), self.gts_t[:, 14, 2].min().item()
        )
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        if "gts_t" in self.__dict__:
            ...
            body_vel_t = (
                1.0 - blend_exp
            ) * body_vel_t0 + blend_exp * body_vel_t1
            body_ang_vel_t = (
                1.0 - blend_exp
        ...
        for f in range(len(motion_data_list)):
            ...
            if not target_heading is None:
                ...
                pose_aa[:, 0] = torch.tensor(
                    (
                        heading_delta * sRot.from_rotvec(pose_aa[:, 0])
                    ).as_rotvec()
                )
