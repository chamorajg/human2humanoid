...
class Stompy_Batch:
    ...
    def __init__(
        self,
        mjcf_file=f"legged_gym/resources/robots/stompypro/robot_test.xml",
        extend_hand=True,
        extend_head=False,
        ...
        if extend_hand:
            ...
            self._parents = torch.cat(
                (mjcf_data["parent_indices"], torch.tensor(self._arm_indices))
            ).to(
                device
        ...
        if extend_head:
            ...
            self._parents = torch.cat(
                (self._parents, torch.tensor(self._root_index).to(device))
            ).to(
                device
            )  # Adding the hands joints
            head_length = 0.5
            self._offsets = torch.cat(
                (
                    self._offsets,
                    torch.tensor([[[0, 0, head_length]]]).to(device),
                ),
                dim=1,
            ).to(device)
            self._local_rotation = torch.cat(
    ...
    def fk_batch(
        ...
        if self.extend_hand and self.extend_head and pose.shape[-2] == 21:
            pose = torch.cat(
                [pose, torch.zeros(B, seq_len, 1, 3).to(device).type(dtype)],
                dim=-2,
