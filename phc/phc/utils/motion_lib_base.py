...
def local_rotation_to_dof_vel(local_rot0, local_rot1, dt):
    ...
    diff_quat_data = torch_utils.quat_mul(
        torch_utils.quat_conjugate(local_rot0), local_rot1
    )
...
class MotionLibBase:
    ...
    def setup_constants(
        ...
        self._success_rate = torch.zeros(self._num_unique_motions).to(
            self._device
        )
        self._sampling_history = torch.zeros(self._num_unique_motions).to(
            self._device
        )
        self._sampling_prob = (
    ...
    def load_motions(
        ...
        for i in range(1, len(jobs)):
            ...
            worker = mp.Process(
                target=self.load_motion_with_skeleton, args=worker_args
            )
            worker.start()
        ...
        self._motion_fps = torch.tensor(
            motion_fps_acc, device=self._device, dtype=torch.float32
        )
        self._motion_bodies = (
            torch.stack(motion_bodies).to(self._device).type(torch.float32)
        )
        self._motion_aa = torch.tensor(
            np.concatenate(motion_aa), device=self._device, dtype=torch.float32
        )

        self._motion_dt = torch.tensor(
            motion_dt, device=self._device, dtype=torch.float32
        )
        self._motion_num_frames = torch.tensor(
            motion_num_frames, device=self._device
        )
        ...
        self.dvs = (
            torch.cat([m.dof_vels for m in motions], dim=0)
            .float()
            .to(self._device)
        )
        if "dof_pos" in motions[0].__dict__:
            self.dof_pos = (
                torch.cat([m.dof_pos for m in motions], dim=0)
                .float()
                .to(self._device)
            )
    ...
    def update_soft_sampling_weight(self, failed_keys):
        ...
        if len(failed_keys) > 0:
            ...
            print(
                f"Training mostly on {len(self._sampling_prob.nonzero())} seqs "
            )
            print(
                self._motion_data_keys[self._sampling_prob.nonzero()].flatten()
            )
            print(
                f"###############################################################################################################################"
            )
    ...
    def update_sampling_prob(self, termination_history):
        if len(termination_history) == len(self._termination_history):
            self._sampling_prob[:] = (
                termination_history / termination_history.sum()
            )
    ...
    def get_motion_num_steps(self, motion_ids=None):
        if motion_ids is None:
            ...
            return (
                self._motion_num_frames[motion_ids] * 30 / self._motion_fps
            ).int()
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        body_ang_vel = (
            1.0 - blend_exp
        ) * body_ang_vel0 + blend_exp * body_ang_vel1
        ...
        if flags.real_traj:
            q_body_ang_vel0, q_body_ang_vel1 = (
                self.q_gavs[f0l],
                self.q_gavs[f1l],
            )
            ...
            q_body_vel = (
                1.0 - blend_exp
            ) * q_body_vel0 + blend_exp * q_body_vel1
